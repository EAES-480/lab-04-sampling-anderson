...
> GM GRADE: 10/10 - outstanding work!
...
Systemic sampling approximated the population mean a bit worse than the SRS, but approximated the SD better. Typically SS performs better in a linear concept, but since this data depends on a cycle pattern, it performed worse in some areas. Depending on the interval value chosen, the sampling might be choosing the same samples multiple times during a cycle. 
...
Values or observations within a cluster are typically very similar to each other, reducing the amount of differing samples. Having a observation that is highly correlated to the others doesn't provoke new information. And, because that data is so similar, it represents a specific part of the data set, not fully encompassing the information of all the observations. 
...
df %>%
  filter(month == "Nov") %>%   # e.g., "Jul" if month is labeled
...
Since we're working with a time series plot and quasi-continuous is made for that type of data, the sampling preserves the patterns. It detects rapid changes sufficiently; provides a full representation of the data that sparse discrete sampling might miss, such as maximum and minimums. 
...
Every time we use a different sampling strategy, we produce a different mean. The one that is closest to the population mean is the Quasi-continuous strategy. The one that is closest to the population SD is the Systemic strategy. 

If there is a cycle process, as seen in this data, systemic data makes sure that the variations within the observations are efficiently seen. You can pick an interval that properly shows and represents the data.

If I were to design a real study, I would choose Quasi-continuous and Systemic sampling. It's not the most time efficient, but also not the least. However, they seem to result in the most precise results, and allows for representation of most of the data.
...
- [X] Setup chunk runs without errors
- [X] Data join happened (df_raw has both canopy + climate columns)
- [X] All chunks run top-to-bottom
- [X] No objects created only in the Console
- [X] Interpretation answers are complete sentences

## After you knit
- [X] Figures appear in the output
- [X] Tables render and are readable
- [X] Your selected x and y are clearly stated in the document
